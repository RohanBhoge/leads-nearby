...
function getCorsHeaders(req: Request) {
    ...
    const isAllowed = isLocalhost || origin === "https://leadsnearby.in";
...
// Plan ID for ₹499/30 days subscription
const PLAN_ID = Deno.env.get("RAZORPAY_PLAN_ID") || "plan_S7oiCw91zxnV96";
...
serve(async (req) => {
    ...
    try {
        // Check for custom header first (bypass gateway verification issue), then fallback to standard
        ...
        if (!token) {
            ...
            return new Response(JSON.stringify({
                error: "Unauthorized: Missing Auth Token (checked X-User-Token and Authorization)",
                receivedHeaders: [...req.headers.keys()]
            }), {
                status: 401,
