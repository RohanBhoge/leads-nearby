...
export const getSMSTemplate = (templateId: string): string => {
  const templates: { [key: string]: string } = {
    [SMS_TEMPLATES.LEAD_COMPLETED.id]: `Hi {{name}}, your {{serviceType}} lead has been completed! Check Leads Near By app for details.`,
    [SMS_TEMPLATES.LEAD_AVAILABLE.id]: `Hi {{name}}, a new {{serviceType}} lead is available in {{location}}. Open Leads Near By to claim it!`,
    [SMS_TEMPLATES.SUBSCRIPTION_EXPIRING.id]: `Hi {{name}}, your Leads Near By subscription expires in {{daysRemaining}} days. Renew now to continue receiving leads!`,
    [SMS_TEMPLATES.LEAD_CLAIMED.id]: `Hi {{name}}, your {{serviceType}} lead has been claimed! You can chat with the agent in the app.`,
    [SMS_TEMPLATES.LEAD_REJECTED.id]: `Hi {{name}}, your {{serviceType}} lead was rejected. View details in Leads Near By app.`,
    [SMS_TEMPLATES.RATING_RECEIVED.id]: `Hi {{name}}, you received a new {{rating}}-star rating! Check your profile in Leads Near By.`,
  };
