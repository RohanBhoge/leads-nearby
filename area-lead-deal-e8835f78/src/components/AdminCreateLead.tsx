      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-md mx-4 max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            <div><label className="text-sm font-medium">Service Type *</label><Select value={form.serviceType} onValueChange={handleSelectChange}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{SERVICE_TYPES.map((type) => (<SelectItem key={type.value} value={type.value}>{type.label}</SelectItem>))}</SelectContent></Select></div>
